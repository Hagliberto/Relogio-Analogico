...
window.addEventListener("DOMContentLoaded", () => {
    // Cria uma instância do relógio e passa o seletor do elemento como argumento
    const c = new Clock(".clock");
  });
  ...
  class Clock {
    ...
    init() {
      // Inicializa o relógio chamando a função timeUpdate()
      this.timeUpdate();
    }
    ...
    get timeAsString() {
      // Obtém a hora como uma string formatada
    ...
    get timeDigits() {
      // Obtém as informações da hora atual como dígitos separados
      ...
      // Atualiza o link da URL
      const urlElement = this.el.querySelector(".clock__ampm-text");
      urlElement.href = "https://hagliberto.github.io/Relogio-Digital/";
  
      return [hh, mm, ss, ap];
    }
  
    animateSecondHand() {
      // Anima o ponteiro dos segundos
      ...
      // Anima os números dentro do ponteiro
    ...
    timeUpdate() {
      // Atualiza o timestamp acessível no atributo `title`
      ...
      // Define os ângulos dos ponteiros no CSS
      for (let a of angles) {
        this.el?.style.setProperty(a.prop, `${a.value}deg`);
      }
  
      // Desenha os dígitos nos ponteiros correspondentes
     ...
     this.animateSecondHand();
      ...
      // Loop para atualizar o relógio a cada segundo
